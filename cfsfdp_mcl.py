...
class DensityPeak:
    ...
    def getDcNew(self, sita):
        ...
        plt.savefig('数据熵1.jpg', dpi=800, bbox_inches="tight")
    ...
    def computePriority(self):
        ...
        plt.grid(lw=0.15,c='lightgray',linestyle='--')
        plt.subplots_adjust(left=0.05, bottom=0.05)
        plt.savefig('决策图1.jpg', dpi=800, bbox_inches="tight")
    ...
    def findn(self):
        ...
        # 取前30个最大的
        max30 = np.argsort(-self.old_representativeness_l)[0:30]
        plt.figure(3)
        plt.title("前m个最大决策值点降序图")
        plt.xlabel("序号")
        plt.ylabel("决策权值")
        plt.plot(range(len(max30)), self.old_representativeness_l[max30], marker='*')
        plt.savefig('决策权值图.svg', dpi=300, bbox_inches="tight")
        # plt.savefig('', format='png')
        # plt.show()
        # 保存偏离度值
        k = []
        te = self.old_representativeness_l[max30]
        for i in range(1, len(max30) - 3):
            # (r_i+1-r_i)(i-1)/(r_i-r1) 注意python中从0开始 所以是*i
            kii1 = self.old_representativeness_l[max30[i + 1]] - self.old_representativeness_l[max30[i]]
            k1i = (self.old_representativeness_l[max30[i]] - self.old_representativeness_l[max30[0]]) / i
    ...
    def findn2(self):
        ...
        plt.savefig('决策权值图.jpg', dpi=800, bbox_inches="tight")
        plt.figure(figsize=(3, 3))
        ...
        plt.savefig('变化率折线图.jpg', dpi=800, bbox_inches="tight")
        # plt.savefig('', format='png')
        # 选择从0——index作为聚类中心
        n = index + 1
        return n
...
if __name__ == '__main__':
    ...
    matplotlib.rcParams['font.family'] = 'sans-serif'  # 使用字体中的无衬线体
    matplotlib.rcParams['font.sans-serif'] = prop.get_name()  # 根据名称设置字体
    matplotlib.rcParams['font.size'] = 9.5  # 设置字体大小
    matplotlib.rcParams['axes.unicode_minus'] = False  # 使坐标轴刻度标签正常显示正负号
    ...
    data = scaler.fit_transform(data_x).tolist()  # 归一化
    ...
    '''
    fig = plt.figure(figsize=(3, 3))
    sns.scatterplot(data=result1, x="x", y="y", palette=sns.color_palette("hls", len(np.unique(data_y))), hue='label',linewidth=0.18)
    ...
    plt.savefig('原始类簇分布图18.jpg', dpi=800, bbox_inches="tight")
    '''
    ...
    plt.title("d.CFSFDP算法聚类结果图", y=-0.28)
    # plt.xlabel("xxxxxx",labelpad=1)
    # plt.ylabel("xxxxxx", labelpad=1)
    # plt.tick_params(pad=100)
    plt.legend(loc='upper left', bbox_to_anchor=(1, 1))
    # plt.subplots_adjust( left=0.8,bottom=0.1)
    plt.savefig('CFSFDP算法聚类图.pdf', dpi=800, bbox_inches='tight')
    # 绘图
    plt.show()
